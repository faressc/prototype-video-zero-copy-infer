...
| gl / vk / wgpu → cpu | `gl_read_pixels` / `vk_map` / `wgpu_map_read` | HOST_COPY | fence wait + `glReadPixels` / `vkMapMemory` (or a staging copy when the export is device-local) / `MapAsync` |
